 ...
 * @author Luis Garcia (lgarciaag89@gmail.com)
 * @see Discretize, increasing range presicion to avoid: A duplicate bin range was detected.
 */
public class CfsSubsetEvalDiscretePrecision extends ASEvaluation
  ...
  /** Constructor */
  public CfsSubsetEvalDiscretePrecision() {
    resetOptions();
  }
  ...
  public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(
        new Option("\tTreat missing values as a separate " + "value.", "M", 0, "-M"));
    newVector.addElement(
        new Option("\tDon't include locally predictive attributes" + ".", "L", 0, "-L"));

    newVector.addElement(new Option("\t" + preComputeCorrelationMatrixTipText(), "Z", 0, "-Z"));

    newVector.addElement(new Option("\t" + poolSizeTipText() + " (default 1)", "P", 1, "-P <int>"));
    newVector.addElement(
        new Option("\t" + numThreadsTipText() + " (default 1)", "E", 1, "-E <int>"));
  ...
  public String preComputeCorrelationMatrixTipText() {
    return "Precompute the full correlation matrix at the outset, "
        ...
        + "parallel processing in order to speed up a backward "
        + "search.";
  }
  ...
  /**
   * Generates an attribute evaluator. Has to initialize all fields of the evaluator that are not
   * being set via options. CFS also discretises attributes (if necessary) and initializes the
   * correlation matrix.
   *
  ...
  public void buildEvaluator(Instances data) throws Exception {
    ...
    if (!m_isNumeric) {
      Discretize m_disTransform = new Discretize();
      m_disTransform.setBinRangePrecision(10);
      m_disTransform.setUseBinNumbers(true);
    ...
    if (m_preComputeCorrelationMatrix && m_poolSize > 1) {
      ...
      numEntriesPerThread = Math.max(numEntriesPerThread, 1);
                ...
                m_pool.submit(
                    () -> {
                      ...
                      for (int i1 = sR; i1 <= eR; i1++) {
                        for (int j1 = (i1 == sR ? sC : 0);
                            j1 < (i1 == eR ? eC : m_corr_matrix[i1].length);
                            j1++) {
                          if (m_corr_matrix[i1][j1] == -999) {
                            float corr = correlate(i1, j1);
                            m_corr_matrix[i1][j1] = corr;
                          }
                        }
                      }
                      if (m_debug) {
                        System.err.println(
                            "Percentage of correlation matrix computed: "
    ...
    for (i = 0; i < nx; i++) {
      for (j = 0; j < ny; j++) {
        ...
        // the class then maximally bad correlation is 0.0
          ...
          if (att1 != m_classIndex && att2 != m_classIndex) {
            r += ((prior_nom[i][j] / m_numInstances));
          }
        }
      }
    }
  ...
  public int[] postProcess(int[] attributeSet) {
    ...
    if (m_debug) {
      System.err.println(
          "Percentage of correlation matrix computed "
              + "over the search: "
              + Utils.doubleToString(((double) m_numFilled.get() / m_numEntries * 100.0), 2)
              + "%");
    }
